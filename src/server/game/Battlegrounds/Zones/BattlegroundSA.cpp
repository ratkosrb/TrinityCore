...
bool BattlegroundSA::ResetObjs()
{
    ...
    for (uint8 i = 0; i < BG_SA_MAX_GY; i++)
    {
        ...
        if (i == BG_SA_BEACH_GY)
        {
            ...
            AddSpiritGuide(i + BG_SA_MAXNPC, sg->Location.GetPositionX(), sg->Location.GetPositionY(), sg->Location.GetPositionZ(), BG_SA_GYOrientation[i], Attackers);
        }
        else
        {
            GraveyardStatus[i] = ((Attackers == TEAM_HORDE)? TEAM_ALLIANCE : TEAM_HORDE);
            if (!AddSpiritGuide(i + BG_SA_MAXNPC, sg->Location.GetPositionX(), sg->Location.GetPositionY(), sg->Location.GetPositionZ(), BG_SA_GYOrientation[i], Attackers == TEAM_HORDE ? TEAM_ALLIANCE : TEAM_HORDE))
...
WorldSafeLocsEntry const* BattlegroundSA::GetClosestGraveYard(Player* player)
{
    ...
    nearest = player->GetExactDistSq(closest->Location.GetPositionX(), closest->Location.GetPositionY(), closest->Location.GetPositionZ());
    ...
    for (uint8 i = BG_SA_RIGHT_CAPTURABLE_GY; i < BG_SA_MAX_GY; i++)
    {
        ...
        dist = player->GetExactDistSq(ret->Location.GetPositionX(), ret->Location.GetPositionY(), ret->Location.GetPositionZ());
        if (dist < nearest)
        {
            closest = ret;
            nearest = dist;
...
void BattlegroundSA::CaptureGraveyard(BG_SA_Graveyards i, Player* Source)
{
    ...
    AddSpiritGuide(i + BG_SA_MAXNPC, sg->Location.GetPositionX(), sg->Location.GetPositionY(), sg->Location.GetPositionZ(), BG_SA_GYOrientation[i], GraveyardStatus[i]);
    uint32 npc = 0;
    uint32 flag = 0;
