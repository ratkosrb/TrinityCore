...
bool BattlegroundEY::SetupBattleground()
{
    ...
    if (!sg || !AddSpiritGuide(EY_SPIRIT_MAIN_ALLIANCE, sg->Location.GetPositionX(), sg->Location.GetPositionY(), sg->Location.GetPositionZ(), 3.124139f, TEAM_ALLIANCE))
    {
    ...
    if (!sg || !AddSpiritGuide(EY_SPIRIT_MAIN_HORDE, sg->Location.GetPositionX(), sg->Location.GetPositionY(), sg->Location.GetPositionZ(), 3.193953f, TEAM_HORDE))
    {
        TC_LOG_ERROR("sql.sql", "BatteGroundEY: Failed to spawn spirit guide. The battleground was not created.");
        return false;
    }
...
void BattlegroundEY::EventTeamCapturedPoint(Player* player, uint32 Point)
{
    ...
    if (!sg || !AddSpiritGuide(Point, sg->Location.GetPositionX(), sg->Location.GetPositionY(), sg->Location.GetPositionZ(), 3.124139f, GetTeamIndexByTeamId(Team)))
...
WorldSafeLocsEntry const* BattlegroundEY::GetClosestGraveYard(Player* player)
{
    ...
    distance = (entry->Location.GetPositionX() - plr_x) * (entry->Location.GetPositionX() - plr_x) +
               (entry->Location.GetPositionY() - plr_y) * (entry->Location.GetPositionY() - plr_y);
