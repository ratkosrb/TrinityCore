...
void BattlegroundMgr::LoadBattlegroundTemplates()
{
    ...
    do
    {
        ...
        if (bgTemplate.Id != BATTLEGROUND_AA && bgTemplate.Id != BATTLEGROUND_RB)
        {
            ...
            {
                bgTemplate.StartLocation[TEAM_ALLIANCE].Relocate(start->Location.GetPositionX(), start->Location.GetPositionY(), start->Location.GetPositionZ(), (start->Location.GetOrientation() * M_PI) / 180);
            }
            else
            {
            ...
            if (WorldSafeLocsEntry const* start = sObjectMgr->GetWorldSafeLoc(startId))
            {
                bgTemplate.StartLocation[TEAM_HORDE].Relocate(start->Location.GetPositionX(), start->Location.GetPositionY(), start->Location.GetPositionZ(), (start->Location.GetOrientation() * M_PI) / 180);
            }
            else
            {
