...
public:
    ...
    */
    ...
    static bool HandleGoGraveyardCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!MapManager::IsValidMapCoord(gy->Location.GetMapId(), gy->Location.GetPositionX(), gy->Location.GetPositionY(), gy->Location.GetPositionZ()))
        {
            handler->PSendSysMessage(LANG_INVALID_TARGET_COORD, gy->Location.GetPositionX(), gy->Location.GetPositionY(), gy->Location.GetMapId());
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        player->TeleportTo(gy->Location.GetMapId(), gy->Location.GetPositionX(), gy->Location.GetPositionY(), gy->Location.GetPositionZ(), (gy->Location.GetOrientation() * M_PI) / 180); // Orientation is initially in degrees
        return true;
    }
